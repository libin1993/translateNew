...
public class LTE_PT_LOGIN {
	...
	public static void loginResp(LTEReceivePackage receivePackage) {
		...
		if (receivePackage.getHost().equals(ServerSocketUtils.REMOTE_4G_HOST) && !TextUtils.isEmpty(equipId)){
		...
		LogUtils.log(receivePackage.getHost()+":登录回复");

		LogUtils.log("TCP发送：IP" + receivePackage.getHost() + "; Type:" +
				sendPackage.getPackageMainType() + ";  SubType:0x" + Integer.toHexString(sendPackage.getPackageSubType())+ ";  子协议:" + Arrays.toString(sendPackage.getByteSubContent()));
